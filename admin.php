    ...
    <body>
        ...
        <div style="display:flex">
            <div class="column" style="padding: 40px 30px 80px 150px">
                ...
                <hr>
                <?php
                    $stmt = $pdo->prepare("SELECT member.fname, member.lname, COUNT(orders.username), COUNT(orders.ord_id) FROM member JOIN orders ON member.username = orders.username GROUP BY member.username");
                    $stmt->execute();
                ?>
                ...
                <?php while($row = $stmt->fetch()): ?>
                    <tr>
                        ...
                        <td><a href="delete.php?ord_id=<?=$row["ord_id"]?>">ลบ</a></td>
                    </tr>
                <?php endwhile; ?>
                </table>
            </div>
            <div class="column" style="padding: 40px 150px 80px 30px">
                ...
                <form method="post" action="admin.php">
                    รหัสสินค้า : <input type="text" placeholder="Search.." name="pid">
